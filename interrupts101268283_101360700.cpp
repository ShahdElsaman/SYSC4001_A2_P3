...
#include "interrupts101268283_101360700.hpp"
    unsigned int next_pid = 1; //update the PCB
...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            (!allocate_memory(&child)); //allocating the memory for the child 

            wait_queue.push_back(current); //parent enter wait quene 
            current = child; 
            ...
            auto [child_exec, child_status, new_time] = simulate_trace(child_trace, current_time, vectors, delays, external_files, current, wait_queue);
            ...
            if (new_size == (unsigned int)-1) {
                std::cerr << "error" << program_name << std::endl;
                execution += "program not found\n";
            ...
            if(!allocate_memory(&current)) {
                std::cerr << "error" << program_name << std::endl;
                execution += "exec is out of memory\n";
            }
